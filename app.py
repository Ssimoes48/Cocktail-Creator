...
def state_data():
    ...
    result_dicts = [{"state": result[0], "abbr": result[1], "latitude": result[2],
                     "longitude": result[3], "cocktail": result[4], "image_scr": result[5]} for result in results]
...
def bubble_data():
    ...
    results = mycursor.fetchall()
    result_dicts = [{"cocktail": result[0], "ingredient": result[1],
                     "measure": result[2], "unit": result[3]} for result in results]
    return jsonify(result_dicts)
​
​
app = Flask(__name__)
